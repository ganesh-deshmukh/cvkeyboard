...
class ContourWithData:
    ...
    def calculateRectTopLeftPointAndWidthAndHeight(
        self
...
def main():
    ...
    try:
        npaClassifications = np.loadtxt(
            "classifications.txt", np.float32
    ...
    try:
        npaFlattenedImages = np.loadtxt(
            "flattened_images.txt", np.float32
    ...
    validContoursWithData.sort(
        key=operator.attrgetter("intRectX")
    ...
    cv2.imshow(
        "imgTestingNumbers", imgTestingNumbers
